...
ffbuild_dockerdl(){ default_dl "$SELF"; to_df "RUN git -C \"$SELF\" fetch --unshallow --filter=blob:none && git -C \"$SELF\" fetch --tags --filter=blob:none"; }

ffbuild_dockerbuild(){
    cd "$FFBUILD_DLDIR/$SELF"
    local myconf=(--prefix="$FFBUILD_PREFIX" --with-jemalloc-prefix=je_ --disable-shared --enable-static)
    ...
    if [[ $VARIANT == *shared* ]]; then
        ...
        rm -f -- "$FFBUILD_PREFIX"/lib/libjemalloc_pic.a
    fi
}

ffbuild_configure(){ echo --custom-allocator=jemalloc; }
